...
from sd_ui_dynamic_prompts import __version__, callbacks
from sd_ui_dynamic_prompts.element_ids import make_element_id
from sd_ui_dynamic_prompts.generator_builder import GeneratorBuilder
from sd_ui_dynamic_prompts.helpers import (
    generate_prompts,
    get_seeds,
...
from sd_ui_dynamic_prompts.paths import (
    get_extension_base_path,
    get_magicprompt_models_txt_path,
    get_wildcard_dir,
)
from sd_ui_dynamic_prompts.pnginfo_saver import PngInfoSaver
from sd_ui_dynamic_prompts.prompt_writer import PromptWriter
from sd_ui_dynamic_prompts.forge_compatibility import apply_forge_patch
...
def _get_install_error_message() -> str | None:
    try:
        from sd_ui_dynamic_prompts.version_tools import get_dynamicprompts_install_result
...
class Script(scripts.Script):
    def __init__(self):
        ...
        # Forge compatibility: ensure settings are registered even if ui_settings_callback hasn't run
        try:
            callbacks.on_ui_settings()
        except Exception:
            pass

        # Forge compatibility: patch prompt_parser.stack_conds for 3D tensors
        try:
            apply_forge_patch()
        except Exception:
            logger.exception("Failed to apply Forge compatibility patch")
